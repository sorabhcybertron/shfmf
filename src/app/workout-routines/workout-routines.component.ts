...
@Component({
	selector: 'workout-routines',
	templateUrl: './workout-routines.component.html',
	styleUrls: ['../../assets/styles/css.css', '../workouts/workouts.component.css', './workout-routines.component.css']
})
export class WorkoutRoutinesComponent implements OnInit {
	...
	public routineworkoutCats: any = [];
	public routineCatsLink: any = 'routines_cats_new_app_json.php?action=getRoutineCats';
	public containerHgt: any;
	public workouts: any;
	public allWorkoutsLoaded: boolean = false;
	public showFilters: boolean = false;
	public showWorkoutFilters: boolean = false;
  ...
  public thisRoute: string = "";
	...
	public loadMoreWorkouts($event, page, displayCount){	// Loads Workouts
	  	...
	  	if(self.workoutsFiltered){
	  		...
	  		this.appService.postCall(this.appService.siteBaseUrl+'routine_filter_json.php?page='+page+'&display='+self.displayCount, this.dt/*, true*/).subscribe(
	  			res => {
	  				self.zone.run(() => {
						console.log(res);
		  	...
		  	self.appService.loadArticles(this.appService.siteBaseUrl+'exercise_json.php?exercOrRoutine=2&user_id='+self.appService.getUserInfo('User Id')+'&page='+page+'&display='+self.displayCount).subscribe(res => {
		  		self.zone.run(() => {
		  			console.log(res);
		  			self.displayFetchedWorkouts(res);
			  	});
	  	...
	  	this.appService.loadArticles(this.routineCatsLink).subscribe(
	  		res => {
	  			if(res.success)
					  ...
					  self.routineworkoutCats = res.workoutcat;
					  self.routineCats.sort(this.compareFilterOptions);
	  		},
	  		err => {
	  			console.log(err);
	...
	filterWorkouts(){
		...
		if(filtersSelected) {	// if any filter is selected
			...
			console.log(self.dt);
			this.appService.postCall(this.appService.siteBaseUrl+'routine_filter_json.php', self.dt/*, true*/).subscribe(
				res => {
					console.log(res);
					if(res.status){
						self.loadError = '';
