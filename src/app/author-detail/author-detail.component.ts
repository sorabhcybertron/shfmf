...
export class AuthorDetailComponent implements OnInit {
  ...
  public viewUserid: null;
	...
	constructor(private location: Location, public appService: AppServicesService,public router :Router,public route:ActivatedRoute) { 
		let ur = "";
		 this.route.url["_value"].forEach(element => {
				ur += element.path+"/";
		 });
		 if(this.appService.admin_url_array.length > 0){
		 	if(this.appService.admin_url_array[this.appService.admin_url_array.length -1] != ur ){
				this.appService.admin_url_array.push(ur);
		 	}
		 }else{ this.appService.admin_url_array.push(ur); }
		
		if(this.route.params["_value"].uname){
			this.appService.getCall('https://muscularstrength.com/get_user_from_username_json.php?username='+this.route.params["_value"].uname).subscribe(
				res => {
					console.log(res);
					if(res.status){
						this.viewUserid = res.id;
						if(this.viewUserid == this.appService.getUserInfo('User Id')){
		...
		}else if(this.route.params["_value"].id){
			this.viewUserid = this.route.params["_value"].id;
			this.loadAccount();
		}else{
			this.router.navigate(['/Dashboard']);
		}
	...
	goBack(){
    	// this.location.back();
    	console.log(this.appService.admin_url_array);
    	if(this.appService.admin_url_array.length > 1){
    		this.appService.admin_url_array.pop();
    		let togo = this.appService.admin_url_array[this.appService.admin_url_array.length - 1];
    		this.router.navigate(['/'+togo]);
    	}else{
	    	if(this.appService.getAuthorUrl()){
	    		this.router.navigate(['/'+this.appService.getAuthorUrl()]);
	    	}else{
	    		this.router.navigate(['/Dashboard']);
	    	}
    	}
	...
	loadAccount(){
		...
		let scope = this;
		this.appService.postCall('https://muscularstrength.com/get_member_info_view_json.php', {mid: this.viewUserid}).subscribe(
			res => {
	...
	postComment(){		// POST NEW COMMENT
		...
		if(this.editorContent.replace(/ /g,'') != ''){	// comment not empty
			// this.editorContent.replace(/(@\S+)/gi,"@<a href='/$1'>$1</a>");
			// this.editorContent.replace(/(\/@)/gi,"/");
			let et = this.appService.ChangeuserCommentLinks(this.editorContent);
			this.appService.postCall('https://muscularstrength.com/get_member_info_view_json.php', {action: 'POST', commenter:this.appService.getUserInfo('User Id'),mid: this.viewUserid, comment: et}).subscribe(
				res => {
					console.log(res);
	...
	postCommentReply(){		// POST NEW COMMENT
		...
		if(this.CommentText != ''){	// comment not empty
			let text = this.appService.ChangeuserCommentLinks(this.CommentText);  //this.CommentText.replace(/(@\S+)/gi,"<a href='/$1'>$1</a>");
			this.appService.postCall('https://muscularstrength.com/get_member_info_view_json.php', {action: 'POST', commenter:this.appService.getUserInfo('User Id'),mid: this.viewUserid, comment: text , postId2: this.postId }).subscribe(
				res => {
					console.log(res);
					if(res.status){		// Comment Posted successfully
						scope.CommentText = '';
	...
	openAlbum(id,type){
		if(id){
			if(type == 'photos'){
				this.router.navigate(["/view-album/photos/"+id+"/"+this.viewUserid]);
			
			}else if(type == 'videos'){
				this.router.navigate(["/view-album/videos/"+id+"/"+this.viewUserid]);
			}
		}
	}
}
